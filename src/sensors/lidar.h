...
struct Lidar
{
	...
	Lidar(std::vector<Car> setCars, double setGroundSlope)    // required arguments to create a member of this class
		: cloud(new pcl::PointCloud<pcl::PointXYZ>()), position(0,0,2.6) // initialize class members
	{        // as soon as a class is initialized the following  will done
		...
		for(double angleVertical = steepestAngle; angleVertical < steepestAngle+angleRange; angleVertical+=angleIncrement)
		{
			...
			{
				Ray ray(position,angle,angleVertical,resoultion); // initilize a Ray class memeber as "ray"
				rays.push_back(ray);
			}
		}
	}
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr scan()  // a function of Lidar class which can be called with pointer -> , and not require any arguments
	{
		cloud->points.clear();
